...
- [x] **ESP-NOW unicast encryption** (PMK/LMK from a shared `nodelink.key`, ≤16 chars; broadcast stays plaintext per ESP-NOW) and **richer leaf heartbeat** — leaves broadcast `heap`/`uptime` over NodeLink, which the gateway forwards into the CMS status. Build-verified on all three targets. *(per-message ack/retry still open.)*
...
- [x] **Synchronized displays / LED effects** — new `sync_action` step broadcasts a *time-scheduled* trigger (`trigger@<epoch>=<id>`) over NodeLink; every node runs the named action at the same cluster-clock instant (via the time-sync above), so LED/display animations fire in unison. ActionEngine gained a scheduler; editable in the Actions UI. Build-verified. *(The shared action drives each node's own outputs; an actual video-wall layout is left to user actions.)*
- [x] **Cluster time sync** — `TimeManager` gets wall-clock from NTP when the node has WiFi; an authoritative node beacons the epoch over NodeLink (`TimeSync`) and infra-less leaves track it against `millis()`. Exposed in `/api/status` (`time`, `timeSource`). Build-verified on all three targets.
...
- [x] **Device groups & tags, bulk operations** — CMS-side tags (`PUT /api/devices/:id/tags`), tag-based targeting for bulk commands + template deploys, fleet-grid tag column + filter, drill-in tag editor.
- [x] **Secure node provisioning/onboarding (QR pairing)** — CMS **Provision** page generates an onboarding QR (`POST /api/provision`) encoding a base64url config bundle into `http://<ap>/?provision=…#/settings`; the device captive portal decodes it and pre-fills Settings for review. Verified end-to-end in a real browser (CMS generates → device frontend lands on Settings pre-filled).
- [x] **Historical telemetry + alerting** in the CMS — per-device rolling telemetry window (`/api/devices/:id/history`, sparklines on the drill-in) and an alert engine (offline + low-heap + low-RSSI thresholds) with a live **Alerts** page, nav badge, `/api/alerts`, and optional outbound **webhook** (`ALERT_WEBHOOK_URL`) for Slack/Discord/generic. Verified end-to-end + covered by the CMS test suite. *(Other Phase D items below remain open.)*
...
- ~~CMS auth model (LAN-trusted vs login)?~~ **Resolved:** open/LAN-trusted by default; set `CMS_AUTH_TOKEN` to require a shared token (Bearer header or HttpOnly login cookie, constant-time compare, login overlay in the UI). Health + firmware downloads stay public for token-less devices. Broker auth + TLS-behind-a-proxy still recommended for wider exposure.
